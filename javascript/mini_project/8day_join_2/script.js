...
form.addEventListener('submit', function (event) {
  ...
  const ssn_front = document.querySelector('#birth');
  const ssn_back = document.querySelector('#reg');
  const username = document.querySelector('#id');
  const password = document.getElementById('password');
  const passwordRetype = document.getElementById('passwordRetype');
  const emailId = document.querySelector('#email1');
  const mailbox = document.querySelector('#email2');
  const address = document.querySelector('#address');
  const gender = document.getElementsByName('gender');
  const agre = document.querySelector('#agree');

  if (id.length < 4 || id.length > 8) {
    alert('아이디를 4~8자 이내로 입력해 주세요.');
    return;
  }
  if (password.innerText !== passwordRetype.innerText) {
